...
|cpu|cpu_stat_container_exter_wait_rate|容器外进程导致的等待率|%|容器| 主线没有，基于 cpu.stat 读取的 throttled_time hierarchy_wait_sum inner_wait_sum 计算 |
|cpu|cpu_stat_container_inner_wait_rate|容器内部进程导致的等待率|%|容器| 主线没有，基于 cpu.stat 读取的 throttled_time hierarchy_wait_sum inner_wait_sum 计算 |
